...
KNHAPI2(knh_Object_t*) new_ReturnObject(CTX ctx, ksfp_t *sfp);
KNHAPI2(knh_RawPtr_t*) new_ReturnCppObject(CTX ctx, ksfp_t *sfp, void *rawptr, knh_Frawfree pfree);
KNHAPI2(void) knh_addConstPool(CTX ctx, knh_Object_t *o);
KNHAPI2(void) knh_write_cid(CTX ctx, knh_OutputStream_t *w, kclass_t cid);
KNHAPI2(void) knh_write_type(CTX ctx, knh_OutputStream_t *w, ktype_t type);
...
KNHAPI2(ktype_t) knh_Method_rtype(CTX ctx, knh_Method_t *mtd, kclass_t this_cid);
KNHAPI2(size_t) knh_Method_psize(knh_Method_t *mtd);
KNHAPI2(ktype_t) knh_Method_ptype(CTX ctx, knh_Method_t *mtd, size_t n, kclass_t this_cid);
KNHAPI2(ktype_t) knh_ParamArray_rtype(knh_ParamArray_t *pa);
KNHAPI2(void) knh_write_mn(CTX ctx, knh_OutputStream_t *w, kmethodn_t mn);
KNHAPI2(kbool_t) Method_isAbstract(knh_Method_t *mtd);
KNHAPI2(void) knh_Func_invoke(CTX ctx, knh_Func_t *fo, ksfp_t *rtnsfp, int argc);
KNHAPI2(void) knh_addTypeMap(CTX ctx, knh_TypeMap_t *tmr, int initCache);
KNHAPI2(void) knh_TypeMap_exec(CTX ctx, knh_TypeMap_t *tmr, ksfp_t *sfp _RIX);
KNHAPI2(knh_TypeMap_t*) new_TypeMap(CTX ctx, kflag_t flag, kclass_t scid, kclass_t tcid, knh_Ftypemap func);
KNHAPI2(knh_TypeMap_t*) new_TypeMapData(CTX ctx, kflag_t flag, kclass_t scid, kclass_t tcid, knh_Ftypemap func, Object *mapdata);
KNHAPI2(knh_TypeMap_t*) knh_findTypeMapNULL(CTX ctx, kclass_t scid0, kclass_t tcid0);
...
KNHAPI2(void) knh_DataMap_setInt(CTX ctx, knh_Map_t *m, const char *key, kint_t value);
KNHAPI2(void) knh_DataMap_setFloat(CTX ctx, knh_Map_t *m, const char *key, kfloat_t value);
...
typedef struct knh_api2_t {
	...
	kbool_t (*Method_isAbstract)(knh_Method_t *mtd);
	kbool_t  (*eval)(CTX ctx, const char *script, kline_t uline, knh_OutputStream_t *w);
	kclass_t (*ktype_tocid)(CTX ctx, ktype_t ptype, kclass_t this_cid);
	knh_Array_t* (*new_Array)(CTX ctx, kclass_t p1, size_t capacity);
	knh_Float_t* (*new_Float)(CTX ctx, kfloat_t value);
	knh_InputStream_t* (*new_InputStream)(CTX ctx, knh_io2_t *io2, knh_Path_t *path);
	knh_Int_t* (*new_Int)(CTX ctx, kint_t value);
	knh_Iterator_t* (*new_IteratorG)(CTX ctx, kclass_t cid, knh_Object_t *source, knh_Fitrnext fnext);
	knh_Map_t* (*new_DataMap)(CTX ctx);
	knh_Object_t* (*new_ReturnObject)(CTX ctx, ksfp_t *sfp);
	...
	knh_RawPtr_t* (*new_ReturnCppObject)(CTX ctx, ksfp_t *sfp, void *rawptr, knh_Frawfree pfree);
	...
	knh_TypeMap_t*  (*findTypeMapNULL)(CTX ctx, kclass_t scid0, kclass_t tcid0);
	knh_TypeMap_t* (*new_TypeMap)(CTX ctx, kflag_t flag, kclass_t scid, kclass_t tcid, knh_Ftypemap func);
	knh_TypeMap_t* (*new_TypeMapData)(CTX ctx, kflag_t flag, kclass_t scid, kclass_t tcid, knh_Ftypemap func, Object *mapdata);
	knh_context_t*  (*getCurrentContext)(void);
	knh_param_t*  (*ParamArray_get)(knh_ParamArray_t *pa, size_t n);
	ktype_t  (*Method_ptype)(CTX ctx, knh_Method_t *mtd, size_t n, kclass_t this_cid);
	ktype_t  (*Method_rtype)(CTX ctx, knh_Method_t *mtd, kclass_t this_cid);
	ktype_t  (*ParamArray_rtype)(knh_ParamArray_t *pa);
	...
	void  (*DataMap_setFloat)(CTX ctx, knh_Map_t *m, const char *key, kfloat_t value);
	void  (*DataMap_setInt)(CTX ctx, knh_Map_t *m, const char *key, kint_t value);
	void  (*DataMap_setString)(CTX ctx, knh_Map_t *m, const char *key, const char *value);
	void  (*Func_invoke)(CTX ctx, knh_Func_t *fo, ksfp_t *rtnsfp, int argc);
	void  (*Object_toNULL_)(CTX ctx, Object *o);
	void  (*OutputStream_flush)(CTX ctx, knh_OutputStream_t *w);
	void  (*OutputStream_p)(CTX ctx, knh_OutputStream_t *w, kbytes_t buf);
	void  (*OutputStream_putc)(CTX ctx, knh_OutputStream_t *w, int ch);
	void  (*OutputStream_write)(CTX ctx, knh_OutputStream_t *w, kbytes_t buf);
	void  (*TypeMap_exec)(CTX ctx, knh_TypeMap_t *tmr, ksfp_t *sfp _RIX);
	...
	void  (*write_cid)(CTX ctx, knh_OutputStream_t *w, kclass_t cid);
	void  (*write_mn)(CTX ctx, knh_OutputStream_t *w, kmethodn_t mn);
	void  (*write_type)(CTX ctx, knh_OutputStream_t *w, ktype_t type);
	void  (*write_utf8)(CTX ctx, knh_OutputStream_t *w, kbytes_t t, int hasUTF8);
} knh_api2_t;
...
void knh_BasicBlock_add_(CTX ctx, knh_BasicBlock_t *bb, kushort_t line, knh_opline_t *op, size_t size);
KMETHOD knh_Fmethod_runVM(CTX ctx, ksfp_t *sfp _RIX);
void knh_PROP(CTX ctx, ksfp_t *sfp, knh_sfpidx_t c, const knh_ClassTBL_t *ct);
void knh_NPROP(CTX ctx, ksfp_t *sfp, knh_sfpidx_t c, const knh_ClassTBL_t *ct);
void knh_PRINT(CTX ctx, ksfp_t *sfp, kflag_t flag, kline_t uline, knh_String_t *msg, knh_Object_t *o);
void knh_PRINTi(CTX ctx, ksfp_t *sfp, kflag_t flag, kline_t uline, knh_String_t *msg, kint_t n);
void knh_PRINTf(CTX ctx, ksfp_t *sfp, kflag_t flag, kline_t uline, knh_String_t *msg, kfloat_t f);
void knh_PRINTb(CTX ctx, ksfp_t *sfp, kflag_t flag, kline_t uline, knh_String_t *msg, kbool_t b);
...
kbool_t knh_Method_ffi(CTX ctx, knh_Method_t *mtd, knh_NameSpace_t *ns, knh_DictMap_t *mdata);
...
knh_Term_t* Tn_typing(CTX ctx, knh_StmtExpr_t *stmt, size_t n, ktype_t reqt, kflag_t opflag);
knh_Term_t* knh_Term_toCID(CTX ctx, knh_Term_t *tk, kclass_t cid);
knh_Term_t* knh_Term_toTYPED(CTX ctx, knh_Term_t *tk, knh_term_t tt, ktype_t type, kshort_t nn);
knh_Term_t* new_TermTYPED(CTX ctx, knh_term_t tt, ktype_t type, kshort_t nn);
kbool_t StmtMETA_is_(CTX ctx, knh_StmtExpr_t *stmt, kbytes_t name);
ksymbol_t Term_fnq(CTX ctx, knh_Term_t *tk);
kclass_t knh_Term_cid(CTX ctx, knh_Term_t *tk, ktype_t reqt);
void knh_GammaBuilder_init(CTX ctx);
kbool_t typingFunction(CTX ctx, knh_Method_t *mtd, knh_StmtExpr_t *stmtB);
void SCRIPT_typing(CTX ctx, knh_StmtExpr_t *stmt);
kbool_t typingMethod2(CTX ctx, knh_Method_t *mtd, knh_StmtExpr_t *stmtB);
...
kbool_t knh_invokeMethod0(CTX ctx, Object *o, knh_NameSpace_t *ns, unsigned long mnd, void **retval, ...);
...
knh_Class_t *new_Type(CTX ctx, ktype_t type);
void knh_write_cname(CTX ctx, knh_OutputStream_t *w, kclass_t cid);
void knh_setClassName(CTX ctx, kclass_t cid, knh_String_t *lname, knh_String_t *snameNULL);
...
void knh_setClassDef(CTX ctx, knh_ClassTBL_t *ct, const kClassDef *cdef);
...
kclass_t knh_addGenericsClass(CTX ctx, kclass_t cid, kclass_t bcid, knh_ParamArray_t *pa);
kclass_t knh_class_Generics(CTX ctx, kclass_t bcid, knh_ParamArray_t *pa);
kclass_t knh_class_P1(CTX ctx, kclass_t bcid, ktype_t p1);
kclass_t knh_class_P2(CTX ctx, kclass_t bcid, ktype_t p1, ktype_t p2);
kbool_t knh_ParamArray_hasTypeVar(knh_ParamArray_t *pa);
void knh_ParamArray_tocid(CTX ctx, knh_ParamArray_t *pa, kclass_t this_cid, knh_ParamArray_t *npa);
knh_ParamArray_t *new_ParamArrayR0(CTX ctx, ktype_t t);
knh_ParamArray_t *new_ParamArrayP1(CTX ctx, ktype_t rtype, ktype_t p1, ksymbol_t fn1);
void knh_ParamArray_add(CTX ctx, knh_ParamArray_t *pa, knh_param_t p);
void knh_ParamArray_addParam(CTX ctx, knh_ParamArray_t *pa, ktype_t type, ksymbol_t fn);
void knh_ParamArray_addReturnType(CTX ctx, knh_ParamArray_t *pa, ktype_t type);
kbool_t knh_ParamArray_equalsType(knh_ParamArray_t *pa, knh_ParamArray_t *pa2);
void knh_Method_setFunc(CTX ctx, knh_Method_t *mtd, knh_Fmethod func);
void knh_Method_toAbstract(CTX ctx, knh_Method_t *mtd);
knh_Method_t* new_Method(CTX ctx, kflag_t flag, kclass_t cid, kmethodn_t mn, knh_Fmethod func);
kindex_t knh_Method_indexOfGetterField(knh_Method_t *o);
kindex_t knh_Method_indexOfSetterField(knh_Method_t *o);
...
knh_Method_t *knh_NameSpace_addXSetter(CTX ctx, knh_NameSpace_t *ns, const knh_ClassTBL_t *ct, ktype_t type, kmethodn_t mn_setter);
void knh_ClassTBL_addMethod(CTX ctx, const knh_ClassTBL_t *t, knh_Method_t *mtd, int isCHECK);
knh_Method_t* knh_ClassTBL_findMethodNULL(CTX ctx, const knh_ClassTBL_t *ct, kmethodn_t mn, int isGEN);
knh_Method_t *knh_ClassTBL_getFmtNULL(CTX ctx, const knh_ClassTBL_t *t, kmethodn_t mn0);
knh_Method_t *knh_ClassTBL_getFmt(CTX ctx, const knh_ClassTBL_t *t, kmethodn_t mn0);
void knh_NameSpace_addMethod(CTX ctx, kclass_t mtd_cid, knh_Method_t *mtd);
knh_Method_t* knh_NameSpace_getMethodNULL(CTX ctx, knh_NameSpace_t *ns, kclass_t cid, kmethodn_t mn);
knh_Method_t* knh_NameSpace_getFmtNULL(CTX ctx, knh_NameSpace_t *ns, kclass_t cid, kmethodn_t mn);
...
knh_TypeMap_t *new_TypeMapMethod(CTX ctx, kflag_t flag, knh_Method_t *mtd);
kbool_t TypeMap_isNoSuchMapping(knh_TypeMap_t *tmr);
...
kbool_t knh_isArrayIterator(knh_Iterator_t *itr);
...
void knh_NameSpace_setLinkClass(CTX ctx, knh_NameSpace_t *ns, kbytes_t linkname, const knh_ClassTBL_t *ct);
const knh_ClassTBL_t *knh_NameSpace_getLinkClassTBLNULL(CTX ctx, knh_NameSpace_t *ns, kbytes_t path, kclass_t tcid);
kclass_t knh_ClassTBL_linkType(CTX ctx, const knh_ClassTBL_t *ct, kclass_t tcid);
knh_Object_t *knh_NameSpace_newObject(CTX ctx, knh_NameSpace_t *ns, knh_String_t *path, kclass_t tcid);
...
knh_Int_t* knh_PtrMap_getI(CTX ctx, knh_PtrMap_t *pm, kunbox_t k);
...
kuintptr_t knh_DictSet_valueAt(knh_DictSet_t *m, size_t n);
kindex_t knh_DictMap_index(knh_DictMap_t *m, kbytes_t key);
Object *knh_DictMap_getNULL(CTX ctx, knh_DictMap_t *m, kbytes_t key);
kuintptr_t knh_DictSet_get(CTX ctx, knh_DictSet_t *m, kbytes_t key);
void knh_DictMap_set_(CTX ctx, knh_DictMap_t *m, knh_String_t *key, dynamic *v);
void knh_DictSet_set(CTX ctx, knh_DictSet_t *m, knh_String_t *key, kuintptr_t n);
void knh_DictMap_append(CTX ctx, knh_DictMap_t *m, knh_String_t *key, knh_Object_t *v);
void knh_DictSet_append(CTX ctx, knh_DictSet_t *m, knh_String_t *key, kuintptr_t n);
...
const kClassDef* knh_getDefaultClassDef(void);
void knh_ClassTBL_setConstPool(CTX ctx, const knh_ClassTBL_t *ct);
void knh_ClassTBL_setObjectCSPI(CTX ctx, knh_ClassTBL_t *ct);
const kClassDef* knh_getCppClassDef(void);
knh_Thunk_t* new_Thunk(CTX ctx, kclass_t p1, size_t envsize);
...
knh_String_t* knh_getPropertyNULL(CTX ctx, kbytes_t key);
...
int knh_addClassConst(CTX ctx, kclass_t cid, knh_String_t* name, Object *value);
...
knh_String_t *knh_getURN(CTX ctx, kuri_t uri);
const knh_PathDPI_t *knh_NameSpace_getStreamDPINULL(CTX ctx, knh_NameSpace_t *ns, kbytes_t path);
const knh_MapDPI_t *knh_NameSpace_getMapDPINULL(CTX ctx, knh_NameSpace_t *ns, kbytes_t path);
const knh_ConverterDPI_t *knh_NameSpace_getConverterDPINULL(CTX ctx, knh_NameSpace_t *ns, kbytes_t path);
