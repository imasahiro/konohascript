...
#define MAX_SIZ 1048576
#define BUF_SIZ 131100
...
int main (int argc, char **argv)
{
   ...
   a = (long*) malloc (BUF_SIZ * sizeof (long));
   b = (long*) malloc (BUF_SIZ * sizeof (long));
 ...
 */
   ...
   for (size = 8; size <= MAX_SIZ; size *= 2) {
      for (i = 0; i < size / 8; i++) {
         a[i] = (long) i;
         b[i] = 0;
      }
      ...
      if (myproc == 0) {
         ...
         MPI_Send(a, size/8, MPI_LONG, other_proc, 0, MPI_COMM_WORLD);
         MPI_Recv(b, size/8, MPI_LONG, other_proc, 0, MPI_COMM_WORLD, &status);
         ...
         MPI_Send(b, size/8, MPI_LONG, other_proc, 0, MPI_COMM_WORLD);
 ...
 */
   ...
   for (size = 8; size <= MAX_SIZ; size *= 2) {
      ...
      MPI_Irecv(b, size/8, MPI_LONG, other_proc, 0, MPI_COMM_WORLD, &request);
      MPI_Barrier(MPI_COMM_WORLD);
      t0 = MPI_Wtime();
 ...
 */
   ...
   for (size = 8; size <= MAX_SIZ; size *= 2) {
      ...
      MPI_Irecv(b, size/8, MPI_LONG, other_proc, 0, MPI_COMM_WORLD, &request_b);
      MPI_Irecv(a, size/8, MPI_LONG, other_proc, 0, MPI_COMM_WORLD, &request_a);
      MPI_Barrier(MPI_COMM_WORLD);
