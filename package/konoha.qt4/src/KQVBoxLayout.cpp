...
bool DummyQVBoxLayout::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQVBoxLayout::event_map->find(str)) == DummyQVBoxLayout::slot_map->end()) {
		bool ret;
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQVBoxLayout::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QVBoxLayout]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQVBoxLayout::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QVBoxLayout]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
...
{
	if (!DummyQVBoxLayout::eventDispatcher(event)) {
