...
bool DummyQTextDocumentFragment::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQTextDocumentFragment::event_map->find(str)) == DummyQTextDocumentFragment::slot_map->end()) {
		bool ret;
		return ret;
	} else {
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQTextDocumentFragment::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QTextDocumentFragment]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQTextDocumentFragment::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QTextDocumentFragment]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
