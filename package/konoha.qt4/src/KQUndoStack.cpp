...
bool DummyQUndoStack::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQUndoStack::event_map->find(str)) == DummyQUndoStack::slot_map->end()) {
		bool ret;
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQUndoStack::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QUndoStack]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQUndoStack::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QUndoStack]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
...
{
	if (!DummyQUndoStack::eventDispatcher(event)) {
