...
bool DummyQValidator::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQValidator::event_map->find(str)) == DummyQValidator::slot_map->end()) {
		bool ret;
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQValidator::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QValidator]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQValidator::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QValidator]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
...
{
	if (!DummyQValidator::eventDispatcher(event)) {
