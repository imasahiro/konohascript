...
bool DummyQTranslator::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQTranslator::event_map->find(str)) == DummyQTranslator::slot_map->end()) {
		bool ret;
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQTranslator::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QTranslator]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQTranslator::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QTranslator]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
...
{
	if (!DummyQTranslator::eventDispatcher(event)) {
