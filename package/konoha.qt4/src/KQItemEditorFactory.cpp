...
bool DummyQItemEditorFactory::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQItemEditorFactory::event_map->find(str)) == DummyQItemEditorFactory::slot_map->end()) {
		bool ret;
		return ret;
	} else {
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQItemEditorFactory::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QItemEditorFactory]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQItemEditorFactory::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QItemEditorFactory]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
