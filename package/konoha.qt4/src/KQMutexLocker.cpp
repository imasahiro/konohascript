...
bool DummyQMutexLocker::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQMutexLocker::event_map->find(str)) == DummyQMutexLocker::slot_map->end()) {
		bool ret;
		return ret;
	} else {
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQMutexLocker::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QMutexLocker]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQMutexLocker::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QMutexLocker]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
