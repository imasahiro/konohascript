...
bool DummyQWriteLocker::signalConnect(knh_Func_t *callback_func, string str)
{
	...
	if ((itr = DummyQWriteLocker::event_map->find(str)) == DummyQWriteLocker::slot_map->end()) {
		bool ret;
		return ret;
	} else {
...
//		KNH_INITv((*(qp->event_map))[event_name], callback_func);
		if (!qp->DummyQWriteLocker::addEvent(callback_func, str)) {
			fprintf(stderr, "WARNING:[QWriteLocker]unknown event name [%s]\n", event_name);
			return;
		}
	}
...
//		KNH_INITv((*(qp->slot_map))[signal_name], callback_func);
		if (!qp->DummyQWriteLocker::signalConnect(callback_func, str)) {
			fprintf(stderr, "WARNING:[QWriteLocker]unknown signal name [%s]\n", signal_name);
			return;
		}
	}
